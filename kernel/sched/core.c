...
// Qingjun.Wang@BSP.KERNEL.PERFORMANCE, 2020/12/09, Add for benchmark
...
void upate_kswapd_unbind_cpu(void)
{
	...
	start_id = rd->mid_cap_orig_cpu;

	if (start_id == -1)
		start_id = rd->max_cap_orig_cpu;
	...
	if (rd->max_cap_orig_cpu == -1)
		end_id = rd->max_cpu_capacity.cpu;
	else
		end_id = rd->max_cap_orig_cpu;
	...
	for (i = start_id; i <= end_id; i++)
		cpumask_set_cpu(i, &kswapd_unbind_cpu);
