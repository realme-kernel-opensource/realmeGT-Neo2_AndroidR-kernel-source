...
#if defined(OPLUS_SF_HWC_BINDER_PERF) && defined(OPLUS_FEATURE_SCHED_ASSIST)
	//NOTE: disallow sf binder to change priority, because they are already set to rt sched class.
	if (thread->proc && thread->proc->proc_type && (task->prio < MAX_RT_PRIO)) {
		return;
	}
#endif /* defined(OPLUS_SF_HWC_BINDER_PERF) && defined(OPLUS_FEATURE_SCHED_ASSIST) */
