...
/* Fangbaio.Yang@BSP.CHG.Basic, 2020/11/25,  Add for AT mode */
#include <soc/oplus/system/boot_mode.h>
#endif

#ifdef OPLUS_CHG_OP_DEF
#define AW8697_LONG_INDEX_HEAD 247
#endif
...
/* used for coloros 7 */
    ...
    {"aw8697_Artist_Notification_RTP_150.bin"},
    {"aw8697_Artist_Ringtong_RTP_151.bin"},
    {"aw8697_Artist_Text_RTP_152.bin"},
    {"aw8697_Artist_Alarm_RTP_153.bin"},
...
static int aw8697_ram_update(struct aw8697 *aw8697)
{
    ...
    if(aw8697->device_id == 815 ) {
        ...
        if (aw8697->f0 < F0_VAL_MIN_0815)
            aw8697->f0 = F0_VAL_MIN_0815;
        else if(aw8697->f0 > F0_VAL_MAX_0815)
            aw8697->f0 = F0_VAL_MAX_0815;
    } else if(aw8697->device_id == 832) {
        if (aw8697->f0 < F0_VAL_MIN_0832)
            aw8697->f0 = F0_VAL_MIN_0832;
        else if(aw8697->f0 > F0_VAL_MAX_0832)
...
static void aw8697_rtp_work_routine(struct work_struct *work)
{
    ...
    if (!rtp_file)
    {
        pr_info("%s  aw8697->rtp_file_num = %d,f0 = %d\n", __func__, aw8697->rtp_file_num, aw8697->f0);
        aw8697->rtp_routine_on = 1;
        if (aw8697->device_id == 815) {
            if (aw8697->f0 <= 1670) {
                ret = request_firmware(&rtp_file,
...
#if 0 //delete by zhengzhiwei@rm.bsp.sensor, this has no use.
...
#ifdef OPLUS_CHG_OP_DEF
	if (aw8697->device_id == 1815 && val >= OPLUS_START_INDEX && val <= OPLUS_END_INDEX) {
		val = val - 49;
    }
#endif
...
static ssize_t aw8697_frequency_show(struct device *dev, struct device_attribute *attr,
        char *buf)
{
#ifdef TIMED_OUTPUT
    struct timed_output_dev *to_dev = dev_get_drvdata(dev);
...
#ifdef OPLUS_CHG_OP_DEF
/* Fangbaio.Yang@BSP.CHG.Basic, 2020/11/25,  Add for AT mode */
    //aw8697->microsecond = 5000000;//temperay solution to ignore osc
#endif
...
/* Fuchun.Liao@BSP.CHG.Basic 2020/04/22 modify for ftm selinux */
...
static DEVICE_ATTR(frequency, S_IRUGO, aw8697_frequency_show, NULL);
