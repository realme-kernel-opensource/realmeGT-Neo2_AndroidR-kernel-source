...
static void ufshcd_init_clk_gating(struct ufs_hba *hba)
{
	...
	gating->gate_wk_in_process = false;
...
/* jason.wu@BSP.Storage, 2020-09-21:
...
#if defined(CONFIG_UFSFEATUR) || defined(CONFIG_SCSI_SKHPB)
...
static int ufshcd_complete_dev_init(struct ufs_hba *hba)
{
	...
	/* jason.wu@BSP.Storage, 2020-06-11:
	 * add for avoid init slowly
	 */
...
static irqreturn_t ufshcd_update_uic_error(struct ufs_hba *hba)
{
	...
	if ((reg & UIC_PHY_ADAPTER_LAYER_ERROR) &&
		...
		dev_dbg(hba->dev, "%s: UIC Lane error reported, reg 0x%x\n",
				__func__, reg);
#ifdef OPLUS_FEATURE_PADL_STATISTICS
/* jason.wu@BSP.Storage, 2020-08-28:
...
//Jinghua.Yu@BSP.Storage.UFS 2020/06/12, Add TAG for UFS plus
		...
		if(hba->dev_info.w_manufacturer_id == 0x1AD && (hba->dev_info.w_spec_version == 0x220 || hba->dev_info.w_spec_version == 0x310))
