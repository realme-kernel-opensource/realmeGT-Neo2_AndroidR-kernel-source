		...
		bool *do_work_lun)
{
	...
	*do_work_lun = false;
	...
	ufsf_para.ufsf->skhpb_lup[lun] = hpb;
	create_skh_hpbfn_enable_proc();
	return 0;
...
static inline void skh_hpbfn_enable_ctrl(struct skhpb_lu *hpb, long val)
{
	switch (val) {
	case 0:
		hpb->force_map_req_disable = true;
		hpb->force_hpb_read_disable = true;
		break;
	case 1:
		hpb->force_map_req_disable = false;
		hpb->force_hpb_read_disable = false;
		break;
	default:
		break;
	}
...
static ssize_t skh_hpbfn_enable_write(struct file *filp, const char *ubuf,
				  size_t cnt, loff_t *data)
{
	...
	for (lun = 0 ; lun < UFS_UPIU_MAX_GENERAL_LUN ; lun++) {
		if (ufsf_para.ufsf->skhpb_lup[lun]) {
			skh_hpbfn_enable_ctrl(ufsf_para.ufsf->skhpb_lup[lun], val);
                }
	}
...
static const struct file_operations skhpbfn_enable_fops = {
	.write = skh_hpbfn_enable_write,
};
