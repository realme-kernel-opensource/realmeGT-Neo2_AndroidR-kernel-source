...
bool t_soc_count = false;
bool g_time_interval = false;
bool t_count_480s = false;
bool t_count_600s = false;
bool t_count_720s = false;
...
#ifdef CONFIG_OPLUS_SHIP_MODE_SUPPORT
		case POWER_SUPPLY_PROP_SHIP_MODE:
			chg_err("====shipmode flag = %d val->intval = %d\n", flag, val->intval);
			if (flag == 0) {
				flag = 1;
...
int oplus_chg_parse_charger_dt(struct oplus_chg_chip *chip)
{
	...
	if (get_eng_version() == HIGH_TEMP_AGING
		...
		chip->limits.max_chg_time_sec = OPCHG_MAX_CHG_TIME_SEC;
	} else {
...
void oplus_chg_set_input_current_limit(struct oplus_chg_chip *chip)
{
	...
	if (chip->pd_chging && current_limit > chip->pd_max_icl)
		current_limit = chip->pd_max_icl;
...
#endif
			if (chip->real_charger_type == POWER_SUPPLY_TYPE_USB_PD)
...
static void oplus_chg_update_ui_soc(struct oplus_chg_chip *chip)
{
	...
	if((!get_soc_feature()) && (g_time_interval==true)){
		g_charger_chip->interval_3 = ktime_to_ms(ktime_get_boottime())/1000;
		g_time_interval = false;
	}
	if((!get_soc_feature()) && (t_soc_count == true) && (chip->ui_soc > chip->soc)){
		g_charger_chip->interval_4 = ktime_to_ms(ktime_get_boottime())/1000;
		if(((g_charger_chip->interval_4 - g_charger_chip->interval_3) <= 740) && (chip->ui_soc < 95) && (200 <= chip->temperature) && (chip->temperature <= 350)){
			if(((chip->ui_soc > chip->soc) && ((g_charger_chip->interval_4 - g_charger_chip->interval_3) >= 480) && ((g_charger_chip->interval_4 - g_charger_chip->interval_3) <= 500)) && (t_count_480s == true )){
				t_count_480s = false;
				chip->ui_soc--;
				printk("delay 8 minute chip->ui_soc=%d \r\n",chip->ui_soc);
			}
			if(((chip->ui_soc > chip->soc) && ((g_charger_chip->interval_4 - g_charger_chip->interval_3) >= 600) && ((g_charger_chip->interval_4 - g_charger_chip->interval_3) <= 620)) && (t_count_600s == true )){
				t_count_600s = false;
				chip->ui_soc--;
				printk("delay 10 minute chip->ui_soc=%d \r\n",chip->ui_soc);
			}
			if(((chip->ui_soc > chip->soc) && ((g_charger_chip->interval_4 - g_charger_chip->interval_3) >= 720) && ((g_charger_chip->interval_4 - g_charger_chip->interval_3) <= 740)) && (t_count_720s == true )){
				t_count_720s = false;
				chip->ui_soc--;
				printk("delay 12 minute chip->ui_soc=%d \r\n",chip->ui_soc);
			}
		} else{
			t_soc_count = false;
			g_charger_chip->interval_3 =0;
			g_charger_chip->interval_4 =0;
			printk("chip->ui_soc=%d chip->soc=%d \r\n",chip->ui_soc,chip->soc);
			if (chip->ui_soc == 100) {
	...
	if (chip->charger_exist && chip->batt_exist && chip->batt_full && chip->mmi_chg && (chip->stop_chg == 1 || chip->charger_type == 5)) {
		...
		printk("smooth_soc=%d chip->ui_soc=%d ",chip->smooth_soc,chip->ui_soc);
		if (chip->smooth_soc <= chip->ui_soc || vbatt_too_low) {
			...
			if (chip->sleep_tm_sec > 0) {
				...
				if (soc_reduce_margin == 0) {
					if ((chip->ui_soc - chip->smooth_soc) > 2) {
						if((t_soc_count == false) || ((1 <= night_count) && (night_count <= 4))){
							chip->ui_soc--;
						}
						soc_down_count = 0;
						chip->sleep_tm_sec = 0;
			...
			printk("soc_down_count=%d soc_down_limit=%d smooth_soc=%d ui_soc=%d t_soc_count=%d night_count=%d \r\n",soc_down_count,soc_down_limit,chip->smooth_soc,chip->ui_soc,t_soc_count,night_count);
	...
	if ((is_batt_full == 1) || (chip->charging_state == CHARGING_STATUS_FULL)
		...
		if ((chip->recharge_after_full == true || chip->vbatt_num == 2)
			...
			if (chip->dod0_counts == DOD0_COUNTS) {
				...
				if ((chip->recharge_after_full == true && chip->recharge_after_ffc == true && oplus_get_flash_screen_ctrl() == false)
					|| (oplus_get_flash_screen_ctrl() == true && chip->chg_ops->charger_suspend_check && chip->chg_ops->charger_suspend_check() == false)) {
					chip->in_rechging = true;
					chip->sw_full_count = 0;
					chip->sw_full = false;
					chip->recharge_after_ffc = false;
...
void oplus_smart_charge_by_shell_temp(struct oplus_chg_chip *chip, int val) {
	...
	if (!val) {
		if (chip->cool_down != 0 && !c_level_index) {
			chip->cool_down = 0;
	...
	case CHARGER_SUBTYPE_QC:
		if (c_level_index <= 0) {
			...
			chip->limits.input_current_cool_down_ma = val <  cool_down_current_limit_normal[c_level_index - 1] ? val : cool_down_current_limit_normal[c_level_index - 1];
			...
			chip->limits.input_current_vooc_ma_high = val <  cool_down_current_limit_vooc[c_level_index - 1] ? val : cool_down_current_limit_vooc[c_level_index - 1];
