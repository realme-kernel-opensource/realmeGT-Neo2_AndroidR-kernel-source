...
static int init_vooc_proc(struct oplus_vooc_chip *chip)
{
	...
	if (get_vooc_mcu_type(chip) == OPLUS_VOOC_MCU_HWID_STM8S) {
		...
		if (get_project() == 136858){
			snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/2169A/oplus_vooc_fw_rk826.bin");
		}else if (get_project() == 136859){
			snprintf(chip->fw_path, MAX_FW_NAME_LENGTH, "fastchg/2169B/oplus_vooc_fw_rk826.bin");
		}else{
