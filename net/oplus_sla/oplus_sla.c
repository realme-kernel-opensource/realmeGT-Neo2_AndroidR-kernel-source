...
/* LiYi@CONNECTIVITY.WIFI.INTERNET, 2020/06/30, Add for router boost */
...
/*SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
...
/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
static int oplus_game_latency_info = 0;
/* OPLUS_SLA_GAME_LATENCY */
...
static void game_app_switch_network(u32 game_type)
{
	...
	bool wlan_bad = false;
	/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
	u32 max_latency = rom_update_info.game_latency;
	/* OPLUS_SLA_GAME_LATENCY */
	if(!oplus_sla_enable){
		return;
	}
	...
	if(cell_quality_good &&!game_cell_to_wifi &&
		(wlan_bad || game_rx_bad || game_rtt >= max_rtt ||
		oplus_game_latency_info >= max_latency) &&
		game_uid[index].mark == MAIN_WLAN_MARK) {
		if(!game_uid[index].switch_time ||
			(time_now - game_uid[index].switch_time) > 60000) {
			/**reinit wifi cell weight status before changing to cellular
			**avoid game app ping through wifi.**/
			init_wifi_cell_weight();
			...
			printk("oplus_sla_game_rtt:uid = %u,game rtt = %u,wlan_bad = %d,game_rx_bad = %d,game_latency = %d,changing to cellular...\n",
				 uid, game_rtt, wlan_bad, game_rx_bad, oplus_game_latency_info);
			/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
			oplus_game_latency_info = 0;
			/* OPLUS_SLA_GAME_LATENCY */
			return;
		}
	}
	...
	if(!wlan_bad &&
		game_uid[index].mark == CELL_MARK &&
		(game_rx_bad || oplus_game_latency_info >= max_latency)) {
		...
		if(!game_uid[index].switch_time ||
			...
			game_bp_info[3] = cell_quality_good;
			oplus_sla_send_to_user(SLA_SWITCH_GAME_NETWORK,(char *)game_bp_info,sizeof(game_bp_info));
			printk("oplus_sla_game_rtt:uid = %u,game rtt = %u,wlan_bad = %d,game_rx_bad = %d, game_latency = %d,changing to wlan...\n",
				 uid, game_rtt, wlan_bad, game_rx_bad, oplus_game_latency_info);
			/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
			oplus_game_latency_info = 0;
			/* OPLUS_SLA_GAME_LATENCY */
...
static int oplus_sla_set_params(struct nlmsghdr *nlh)
{
	...
	if (count == 14) {
		...
		/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
		rom_update_info.game_latency = params[13];
		/* OPLUS_SLA_GAME_LATENCY */
...
/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
static void oplus_sla_set_game_latency(struct nlmsghdr *nlh)
{
	u32 *data = (u32 *)NLMSG_DATA(nlh);
	oplus_game_latency_info = data[0];
	printk("oplus_sla_set_game_latency: oplus_game_latency_info [%d]\n", oplus_game_latency_info);
}
/* OPLUS_SLA_GAME_LATENCY */
...
static int sla_netlink_rcv_msg(struct sk_buff *skb, struct nlmsghdr *nlh, struct netlink_ext_ack *extack)
{
	...
	#endif /* OPLUS_FEATURE_WIFI_ROUTERBOOST */
	/* SuPei@CONNECTIVITY.WIFI.NETWORK.SLA.1379767 2021/04/25, Add for game latency */
	case SLA_SET_GAME_LATENCY:
		oplus_sla_set_game_latency(nlh);
		break;
	/* OPLUS_SLA_GAME_LATENCY */
	default:
		return -EINVAL;
