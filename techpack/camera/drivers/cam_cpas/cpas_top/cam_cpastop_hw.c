...
/*yufeng@camera, 20210309, vfe overflow case 05108601 */
	struct cam_cpas *cpas_core = (struct cam_cpas *) cpas_hw->core_info;
	struct cam_hw_soc_info *soc_info = &cpas_hw->soc_info;
	int reg_base_index;
#endif
	...
	/* Update the cam_noc_amm_jpeg_qosgen_MainCtl_Low  */
	cam_io_w_mb(0x2, soc_info->reg_map[reg_base_index].mem_base +
		0x2D08);
	/* Update the cam_noc_amm_jpeg_qosgen_Shaping_Low  */
	cam_io_w_mb(0x8080808, soc_info->reg_map[reg_base_index].mem_base +
		0x2D20);
	/* Update the cam_noc_amm_jpeg_qosgen_Shaping_High	 */
	cam_io_w_mb(0x8080808, soc_info->reg_map[reg_base_index].mem_base +
		0x2D24);
