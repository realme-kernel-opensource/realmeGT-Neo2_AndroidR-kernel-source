...
//	}
    /*else{
		if (emap[j].mem.valid_size) {
            CAM_ERR(CAM_EEPROM, "sid %d",e_ctrl->io_master_info.cci_client->sid);
	...
	struct cam_write_eeprom_t *cam_write_eeprom)
{
	...
	CAM_DBG(CAM_EEPROM, "write start, cam: ID: 0x%x, reg_addr: 0x%x, val: %d",
	...
	CAM_DBG(CAM_EEPROM, "exit write eeprom !!!");
	...
	if ((check_eeprom_data.cam_id == 0x01 || check_eeprom_data.cam_id == 0x02  || check_eeprom_data.cam_id == 0x03) &&
		...
		for (i = 0; i < check_eeprom_data.checkDataSize; i++ ){
			rc = camera_io_dev_read(&e_ctrl->io_master_info,
				...
				CAMERA_SENSOR_I2C_TYPE_WORD,
				CAMERA_SENSOR_I2C_TYPE_BYTE);
            CAM_ERR(CAM_EEPROM, "eeprom read  rc %d",rc);
			if (rc) {
...
int32_t oplus_cam_eeprom_driver_cmd(struct cam_eeprom_ctrl_t *e_ctrl, void *arg)
{
	int                            rc = 0;
	struct cam_control            *cmd = (struct cam_control *)arg;
	switch (cmd->op_code) {
		...
		case CAM_WRITE_AE_SYNC_DATA:
			...
			break;
		default:
			CAM_ERR(CAM_EEPROM, "finish cam_eeprom_driver_cmd");
			break;
#endif
	}
	return rc;
