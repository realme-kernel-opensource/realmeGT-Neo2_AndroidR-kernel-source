...
int cam_flash_pmic_pkt_parser(struct cam_flash_ctrl *fctrl, void *arg)
{
	...
	case CAM_FLASH_PACKET_OPCODE_INIT: {
		...
		case CAMERA_SENSOR_FLASH_CMD_TYPE_INIT_FIRE: {
			...
			rc = fctrl->func_tbl.apply_setting(fctrl, 0);
#ifdef OPLUS_FEATURE_CAMERA_COMMON
			/* honglin.xiao@Camera.Drv fix the flash_state stay in CAM_FLASH_STATE_CONFIG
			 *
			 * when from the first flash switch to the second flash, the second flash state
			 * will be transition from CAM_FLASH_STATE_START to CAM_FLASH_STATE_CONFIG and
			 * never be recovery to CAM_FLASH_STATE_START.
			 */
			if (rc) {
				CAM_ERR(CAM_FLASH,
