	...
	bool protect, int32_t offset)
{
	...
	/*Guobao.Xiao@Camera, 20200415 add debug log, case:04544792*/
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		/*Guobao.Xiao@Camera, 20200415 add for solve dump issue of secure camera, case:04544792*/
		offset = cam_csiphy_get_instance_offset(csiphy_dev,
			release.dev_handle);
		if (offset < 0 || offset >= CSIPHY_MAX_INSTANCES) {
			CAM_ERR(CAM_CSIPHY, "Invalid offset");
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
			//wangyongwu@Camera.Drv modify for Need to stop the cpas for csiphy if start cpas failed case:04389611
			if (rc == -EALREADY)
				cam_cpas_stop(csiphy_dev->cpas_handle);
#endif
			goto release_mutex;
...
#ifdef OPLUS_FEATURE_CAMERA_COMMON
		CAM_INFO(CAM_CSIPHY, "csiphy:%d is enabled", csiphy_dev->soc_info.index);
#endif
	}
		break;
