...
#if defined(OPLUS_FEATURE_PXLW_IRIS5)
	if ((iris_get_feature() || iris_soft_get_feature()) && sde_enc->num_phys_encs > 0)
...
#if defined(OPLUS_FEATURE_PXLW_IRIS5)
	if (iris_soft_get_feature() && sde_enc->num_phys_encs > 0)
		iris_sync_panel_brightness(3, sde_enc->phys_encs[0]);
#endif
...
#if defined(OPLUS_FEATURE_PXLW_IRIS5)
	if (iris_soft_get_feature() && sde_enc->num_phys_encs > 0 && event == MSM_ENC_COMMIT_DONE)
		iris_sync_panel_brightness(4, sde_enc->phys_encs[0]);
#endif
	return ret;
}
