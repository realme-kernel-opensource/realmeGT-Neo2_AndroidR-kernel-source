...
/* Lauwo.Zhong@MM.Display.LCD.Feature,2021-01-15 add for adfr hardware revision compatibility */
...
int oplus_adfr_adjust_tearcheck_for_dynamic_qsync(void *sde_phys_enc)
{
	...
	if (sde_connector_get_qsync_mode(phys_enc->connector) == 0 ||
		...
		/* ZhongLiuhe@MULTIMEDIA.DISPLAY.LCD.FEATURE, 2021/01/25, Fixed qsync window and panel min fps nonsynchronous issue */
...
static int oplus_dsi_display_vsync_switch_check_te(struct dsi_display *display, int level)
{
	...
	/* Lauwo.Zhong@MM.Display.LCD.Feature,2021-01-18 add for Filter out all vsync switch */
...
/* ZhongLiuhe@MULTIMEDIA.DISPLAY.LCD.FEATURE, 2021/02/01, Add for vsync switch status reset */
...
int oplus_adfr_handle_auto_mode(u32 propval)
{
	...
	if (propval & OPLUS_ADFR_AUTO_MODE_MAGIC) {
		/* ZhongLiuhe@MULTIMEDIA.DISPLAY.LCD.FEATURE, 2021/02/26, filter repeat auto mode setting */
